...
void any_msg_set_location(uint8_t *ts_field, int32_t ts) {
  int i;
  for (i = 0; i < ANC_LOCATION_LENTH; i++) {
    ts_field[i] = (uint8_t) ts;
    ts >>= 8;
  }
}

void any_msg_get_location(const uint8_t *ts_field, int32_t *ts) {
  int i;
  *ts = 0;
  for (i = 0; i < ANC_LOCATION_LENTH; i++) {
    *ts += ((int32_t)ts_field[i]) << (i * 8);
  }
}
...
//  *ts = 0;
//  for (i = 0; i < ANC_LOCATION_LENTH; i++) {
//    *ts += ((int32_t)ts_field[i]) << ((ANC_LOCATION_LENTH-i-1) * 8);
//  }
//}
